...
def notify_people_on_next_task(task, people):
	...
	company_name = Company.query.get(order.company_id).name
	if step_number > 2:
		content = f"Steps 1 to {step_number-1} in the order titled '{service_name}', created on {order_creation_time}, for the client called '{client_name}' have been completed. You can now begin working on step {step_number} titled '{task_title}'."
	else:
