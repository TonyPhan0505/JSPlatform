...
def delete_order(order_id, decision):
	if decision == 'PENDING':
		...
		try:
			...
			db.session.rollback()
		content = f"The order titled {order_name}, created on {time_created}, for the client called {client_name} has been deleted by {current_user.name} ({current_user.email}) from the {Department.query.get(current_user.department_id).name} department."
